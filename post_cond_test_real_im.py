...
def transorm_for_classifier(ims):
    # Resize the tensor (using interpolation)
    ims_resized = F.resize(ims, (256, 256))

    # Normalize the tensor
    im_tensor = F.normalize(ims_resized, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])

    im_tensor = (2 * im_tensor) - 1

    return im_tensor

def predict_with_oracle(model, ims):
